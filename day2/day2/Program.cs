    ...
    class Program
    {
        ...
        static int Part1()
        {
            ...
            foreach (string boxID in boxIDS)
            {
                ...
                {
                    if(letters.TryGetValue(character, out int temp))
                    {
                        letters[character] += 1;
                    } else
                    {
                ...
                bool hasTwoAlready = false;
                bool hasThreeAlready = false;
                foreach (KeyValuePair<char, int> entry in letters)
                {
                    
                    if(entry.Value == 2 && !hasTwoAlready)
                    {
                        doubles++;
                        hasTwoAlready = true;
                    }
                    if(entry.Value == 3 && !hasThreeAlready)
                    {
                        triples++;
